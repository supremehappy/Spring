...
<form:form modelAttribute="user" action="#" method="post"
	...
	<table border="1" width="450" align="center">	
		...
		<tr><td>E-Mail</td>
		<td><input type="email" name="email" size="30"  value="${USER_SELECT.email}"/>
		</td>		
		</tr>
		...
		<c:if test="${!empty INTEREST_GENRES}">
			...
			<c:choose>
				<c:when test="${interest.genre_id == i.count}">
					<input type="checkbox" name="genre_arr" value="${interest.genre_id}" checked>${interest.genre_name} &nbsp;
				</c:when>
			<c:otherwise>
				<input type="checkbox" name="genre_arr" value="${interest.genre_id}" checked>${interest.genre_name} &nbsp;
			</c:otherwise>
...
<%-- 			<c:otherwise>
		...
		<tr><td>프로필 사진</td>
		<td><input type="file" name="picture_file" size="30"/></td>
		</tr>		
		
		<tr><td>자기소개</td>
		<td><form:textarea cols="42" rows="8" path="introduce"/></td>	
		</tr>
