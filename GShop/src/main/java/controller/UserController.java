   ...
   @RequestMapping(value="/user/userInput.html",
	...
	public ModelAndView userInp(@Valid User user,
		...
		if(multiFile != null){//업로드 이미지가 있는 경우
			...
			path = context.getRealPath("../gshop/user/upload/"+fileName);
			try{
			...
			user.setPicture_name(fileName);//이미지파일 이름 설정
		}
		...
		ModelAndView mav = new ModelAndView("gshop/template");
		mav.addObject("BODY","/user/userMaPage.html");
		return mav;
	}
	...
	@RequestMapping(value="/user/nicknameOverlapCheck.html",
			method=RequestMethod.POST)
	public ModelAndView nicknameOverlapCheck(HttpServletRequest request){
		User user = new User();		
